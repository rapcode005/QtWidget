...
public:
    ...
    void setValuesGOEA(const QString &val1, const QString &val2, const double &val3);
    void setValuesWBPHS(const QString &val1, const QString &val2, const QString &val3, const double &val4);
    ...
    QList<QStringList> calculateMissingSec(QStringList flightpath);
    //QList<QStringList> missinglist;
    void saveGps(QStringList flightpath, QList<QStringList> missinglist, QString filename);
    QStringList defaultUsedHotKeys;
    void validateConditionalValueFields(const QString &arg, QString sType, bool boolWarning);
    void conditionalFieldValuesEnabled(QList<QStringList> &headerList, bool boolEnableControl, bool boolSetVaues);
    QList<QStringList> tempHeaderList;
    QList<QStringList> getHeaderConditionalValues(const QString &arg, QString category, QString sField);
    QList<QStringList> tempHeaderConditionalVals;

    QList<LogTemplateItem*> speciesItems;//added 8.20.2020
    QList<LogTemplateItem*> groupingItems;//added 8.20.2020
    QSettings *scribeSettings;//added 8.26.2020
    void saveScribeSettings(bool saveAsc);
...
public slots:
    ...
    void onMaximizeMap(QString str);
    QList<QStringList> getConditionalValueFieldsPrevValues(QList<QStringList> &headerList);
    void checkBCRValue(const QString& bcrval);//added 8.17.2020
...
signals:
    ...
    void receivedLogDialogItemsWithoutSlot(); //added 8.20.2020
    void receivedLogDialogItems(); //added 8.20.2020
...
private slots:
    ...
    void on_observation_doubleClicked(int row, int column);
    ...
    void on_actionExport_Map_to_KML_triggered();

    void on_actionInterpolate_Flight_GPS_Data_triggered();

    //void on_pushSpHot_clicked();

    void action_editSpeciesClicked(); //added 8.20.2020

    void action_editGroupingClicked(); //added 8.20.2020
...
private:
    ...
    interpolategps                 *interpolatedialog;

    LogTemplateItem *currentSpecies;
    LogTemplateItem *currentSpeciesOb;
    QString ascFilename = "";
    GpsHandler *gps;
    AudioPlayer *audio;
    QFile sourceFile;
    ...
    QStringList previousWBPS;
    //QString previousTransect, previousBCR;
    QString previousAGName, previousPlotName;
    ...
    //HeaderFields Slot
    QList<QMetaObject::Connection> connection, connectionA, connectionObservation;
    ...
    QString populateKMLObservation(QStringList flds,QStringList obs, int idxtitle, QString style);
    QString populateKMLFlightPath(QString strtitle, QString style);

    void adjustHeaderSize();
    void adjustAdditionalSize();

    //Observation
    int obRow, obCol;
    struct objectObs {
        int transectIndex, stratumIndex, segmentIndex, bcrIndex;
    };
    objectObs obsIndex;
    QString obVal, preVal;
    QStringList previousObTrnSeg, previousTrnBcr;

    void disconnectObservation();
    bool condition(const QString &grouping, const QString &quantity);

    bool pressDoubleClick = false;

    bool boolFreshLoad = false;

    //int getIndexFromHeaderFields(const QString &str);

    LogItemEditDialog *speciesDialog;//added 8.30.2020
    LogItemEditDialog *groupingDialog;//added 8.30.2020
};
